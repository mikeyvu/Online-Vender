		...
		<table class="tbl-full">
			...
			<c:forEach var="c" items="${categories}">
				<tr>
					...
					<td>${c.title}</td>
					<td>
						<img src="/assets/img/category/${c.imageName }" alt="Category Image">
					</td>
					<td>${c.featured}</td>
					<td>${c.active}</td>
					<td>
						<a href="<%= request.getContextPath() %>/updateCategoryServlet?id=${c.id }" class="btn-secondary">Update Category</a> 
						<a href="<%= request.getContextPath() %>/manageCategoryServlet?action=DELETE&id=${c.id}&image_name=${c.imageName}" class="btn-danger">Delete Category</a>
					</td>
				</tr>
